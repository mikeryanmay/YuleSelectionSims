...
# setwd("simulations/empirical_num/")

library(gtools)

# clear the _run directory
old_runs <- list.files("_runs", full.names = TRUE)
file.remove(old_runs)
dir.create("_runs", showWarnings = FALSE)
...
for(i in 1:num_chunks) {
  ...
  writeLines(this_slurm, paste0("_runs/", paste0("chunk_", i), ".sh" ))
