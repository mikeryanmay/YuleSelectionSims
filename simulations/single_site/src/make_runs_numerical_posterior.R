...
old_runs <- list.files("_run_numerical_posterior", full.names = TRUE)
file.remove(old_runs)
dir.create("_run_numerical_posterior", showWarnings = FALSE)
...
all_dirs <- all_dirs[sapply(all_dirs, function(x) any(grepl("numerical_posterior.tsv", list.files(x)))) == FALSE]

# sort the jobs naturally
all_dirs <- mixedsort(all_dirs)

# read template
template <- readLines("src/run_template_numerical_posterior.sh")
...
for(i in 1:num_chunks) {
  ...
  writeLines(this_slurm, paste0("_run_numerical_posterior/", paste0("chunk_", i), ".sh" ))
