...
old_runs <- list.files("_run_liks", full.names = TRUE)
file.remove(old_runs)
dir.create("_run_liks", showWarnings = FALSE)
...
# check if the runs already have liks.tsv
all_dirs <- all_dirs[sapply(all_dirs, function(x) any(grepl("liks.tsv", list.files(x)))) == FALSE]

# sort the jobs naturally
all_dirs <- mixedsort(all_dirs)

# read template
template <- readLines("src/run_template_liks.sh")
...
for(i in 1:num_chunks) {
  ...
  writeLines(this_slurm, paste0("_run_liks/", paste0("chunk_", i), ".sh" ))
