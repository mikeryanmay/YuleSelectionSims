...
summaries <- do.call(rbind, lapply(1:nrow(grid), function(i) {
  ...
  this_ml_tsv  <- paste0("factor/tips_", this_tips, "_size_", this_size, "_factor_", this_factor, "/rep_", this_rep, "/marg_lik.tsv")
  this_fix_tsv <- paste0("factor/tips_", this_tips, "_size_", this_size, "_factor_", this_factor, "/rep_", this_rep, "/partitioned.tsv")
  if ( file.exists(this_ml_tsv) == FALSE | file.exists(this_fix_tsv) == FALSE ) {
    return(NULL)
  }
  
  # read output files
  ml_results  <- read.table(this_ml_tsv, header = TRUE, sep = "\t", stringsAsFactors = FALSE)
  fix_results <- read.table(this_fix_tsv, header = TRUE, sep = "\t", stringsAsFactors = FALSE)
  ...
  # compute bayes factor with unknown parameter
  joint_likelihood_selected <- ml_results[1,1]
  joint_likelihood_constant <- ml_results[2,1]
  ...
  joint_posteriors_ml <- exp(joint_posteriors)
  
  # compute bayes factor with known parameter
  joint_likelihood_selected <- fix_results[3,1]
  joint_likelihood_constant <- fix_results[5,1]
  joint_likelihoods <- c(joint_likelihood_selected, joint_likelihood_constant)
  joint_likelihoods <- joint_likelihoods - max(joint_likelihoods)
  joint_posteriors  <- joint_likelihoods + log(priors)
  joint_posteriors  <- joint_posteriors - log(sum(exp(joint_posteriors)))
  ...
  # compute the raw posteriors
  joint_posteriors_fix <- exp(joint_posteriors)
  ...
  res <- data.frame(tips        = this_tips,
                    ...
                    factor      = this_factor, 
                    rep         = this_rep,
                    ml_post     = I(list(joint_posteriors_ml)),
                    fix_post    = I(list(joint_posteriors_fix)))
...
pdf(paste0(figdir, "posterior_fixed_vs_unknown.pdf"), height = 2.8, width = 12)
layout(layout_mat)
...
for(i in 1:length(factors)) {
  ...
  these_ml_post  <- do.call(rbind, these_summaries$ml_post)[,1]
  these_fix_post <- do.call(rbind, these_summaries$fix_post)[,1]
  df             <- data.frame(c = these_tips, m = these_ml_post, f = these_fix_post)
