...
old_runs <- list.files("_run_posteriors", full.names = TRUE)
file.remove(old_runs)
dir.create("_run_posteriors", showWarnings = FALSE)
...
all_dirs <- all_dirs[sapply(all_dirs, function(x) any(grepl("posterior.log", list.files(x)))) == FALSE]

# sort the jobs naturally
all_dirs <- mixedsort(all_dirs)

# read template
template <- readLines("src/run_template_posterior.sh")
...
for(i in 1:num_chunks) {
  ...
  writeLines(this_slurm, paste0("_run_posteriors/", paste0("chunk_", i), ".sh" ))
