...
old_runs <- list.files("_run_fits", full.names = TRUE)
file.remove(old_runs)
dir.create("_run_fits", showWarnings = FALSE)
...
# read template
template <- readLines("src/run_template_fits.sh")
...
for(i in 1:num_chunks) {
  ...
  writeLines(this_slurm, paste0("_run_fits/", paste0("chunk_", i), ".sh" ))
