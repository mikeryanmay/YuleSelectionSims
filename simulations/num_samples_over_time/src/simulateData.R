...
simulateYuleSelectionTime <- function(S, lambda0, delta, phi, gamma, t, max_size = 1000, model = "additive", verbose = FALSE) {
  ...
  repeat {
    ...
    # simulate
    sim <- simulateYuleSelectionTreeTime(S, lambda0, delta, phi, gamma, t, model)  
    if ( is.null(sim)  ) {
    ...
    if ( sum(num_gains) == 1 & length(sim$phy$tip.label) <= max_size ) {
      break
    }
...
simulateYuleSelectionTreeTime <- function(S, lambda0, delta, phi, gamma, t, model = "additive") {
  ...
  # compute the (maximum) number of lineages (including the stem)
  lineage_chunk <- 1000 # TODO: check that this is large enough
  
  # pre-allocate the lineage container
  lineages <- data.frame(anc = rep(NA, lineage_chunk), desc = NA, start = NA, end = NA, state = NA, fitness = NA, active = FALSE)
  
  # getFitnessForSequence(init_state, fit_state, lambda0, delta, model)
  # tmp <- init_state
  # substr(tmp, 1, 10) <- paste0(rep("A", 10), collapse = "")
  # getFitnessForSequence(tmp, fit_state, lambda0, delta, model)
  ...
  # create the sequence alignment
  seq <- matrix(lineages$state[lineages$desc <= num_tips], ncol = 1)
  rownames(seq) <- paste0("t_", lineages$desc[lineages$desc <= num_tips])
  seq <- t(t(seq[tip.label,]))
  
  # get sequences for nodes
  nodes <- sort(lineages$desc[lineages$desc > num_tips])
  nodeseq <- matrix(lineages$state[match(nodes, lineages$desc)], ncol = 1)
  rownames(nodeseq) <- nodes
